...
;; Toggles
(bind-key "C-c t a" 'my-toggle-auto-hscroll-mode)
(bind-key "C-c t b" 'scroll-bar-mode)
(bind-key "C-c t c" 'flycheck-mode)
(bind-key "C-c t f" 'variable-pitch-mode)
(bind-key "C-c t h" 'global-hl-line-mode)
(bind-key "C-c t l" 'visual-line-mode)
(bind-key "C-c t m" 'menu-bar-mode)
(bind-key "C-c t n" 'my-toggle-line-numbers)
(bind-key "C-c t o" 'overwrite-mode)
(bind-key "C-c t s" 'flyspell-mode)
(bind-key "C-c t t" 'toggle-truncate-lines)
(bind-key "C-c t v" 'visual-fill-column-mode)
...
(use-package eglot
  ...
  (defun my-eglot-hook()
    (when (eglot-managed-p)
      ;; TODO: order causing "requires version X, you have version Y"
...
(use-package winner
  :bind (("C-c w" . winner-undo)
         ("C-c W" . winner-redo))
  :config (setq winner-dont-bind-my-keys t) ; don't bind C-<left|right>
  :init (winner-mode))
#+end_src
