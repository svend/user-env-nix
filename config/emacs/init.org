...
(let ((deps-path (car exec-path))
  ...
  (dolist (p (reverse (list deps-path nix-profile-path local-path)))
    (setq exec-path (cons p (remove p exec-path))))

  (setenv "PATH" (string-join
                  (list deps-path nix-profile-path local-path path-env)
                  ":")))
...
(use-package eglot
  ...
  ;; End of https://github.com/joaotavora/eglot/issues/574 workaround
...
(use-package hydra
  ...
  (global-set-key
   ...
   (defhydra hydra-toggle (:timout 10)
     "Toggle"
     ("a" my-toggle-auto-hscroll-mode "hscroll-mode")
